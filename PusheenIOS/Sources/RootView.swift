...
struct PusheenTabs: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            HStack(spacing: 3) { ForEach(Array(items.enumerated()), id: \.offset) { index, item in Button { withAnimation(.spring(response: 0.34, dampingFraction: 0.84)) { selection = index } } label: { VStack(spacing: 4) { Image(systemName: item.1).font(.title3.weight(.semibold)); Text(item.0).font(.caption2.weight(.semibold)).lineLimit(1) }.frame(minWidth: 78).foregroundStyle(selection == index ? Color.pink.opacity(0.92) : .white.opacity(0.88)).padding(.vertical, 9).background(selection == index ? Color.white.opacity(0.14) : .clear, in: Capsule()) }.buttonStyle(.plain) } }
                .padding(5).liquidCard(Capsule()).padding(.horizontal, 42).padding(.bottom, 12).offset(x: dragX)
